...
description: 如何在运行 [!DNL Analysis Workspace] 活动时配置[!UICONTROL Auto-Target]中的A4T报告以获取预期结果？
...
[!UICONTROL Analytics for Target]活动的[!DNL Auto-Target] (A4T)集成使用[!DNL Adobe Target]集成机器学习(ML)算法，根据每位访客的个人资料、行为和上下文选择最佳体验，同时使用[!DNL Adobe Analytics]目标量度。

尽管[!DNL Adobe Analytics] [!DNL Analysis Workspace]中提供了丰富的分析功能，但由于实验活动（手动&#x200B;**[!UICONTROL Analytics for Target]**&#x200B;和[!DNL Auto-Target]）和个性化活动([!UICONTROL A/B Test])之间存在差异，因此需要对默认[!UICONTROL Auto-Allocate]面板进行一些修改才能正确解释[!UICONTROL [!UICONTROL Auto-Target]]活动。

本教程介绍了为分析[!UICONTROL Auto-Target]中的[!DNL Analysis Workspace]活动而建议的修改，这些修改基于以下关键概念：
...
## 在[!UICONTROL Auto-Target]中为[!DNL Analysis Workspace]面板创建A4T

要为[!UICONTROL Auto-Target]报表创建A4T，请在&#x200B;**[!UICONTROL Analytics for Target]**&#x200B;中从[!DNL Analysis Workspace]面板开始（如下所示），或以自由格式表开始。 然后，进行以下选择：
...
3. **[!UICONTROL Success Metrics]**：虽然您可以选择要报告的任何指标，但通常应查看与在[!DNL Target]中创建活动期间选择优化的同一指标有关的报告。
   ...
   ![[!UICONTROL Analytics for Target]活动的[!UICONTROL Auto-Target]面板设置。](assets/Figure1.png)

   *图1：[!UICONTROL Analytics for Target]活动的[!UICONTROL Auto-Target]面板设置。*

>[!TIP]
>
>要为[!UICONTROL Analytics for Target]活动设置[!UICONTROL Auto-Target]面板，请选择任何控制体验，选择[!UICONTROL Visits]作为标准化量度，并选择在创建[!DNL Target]活动期间为优化选择的相同目标量度。
...
>[!NOTE]
>
>当前，[!UICONTROL Lift and Confidence]的A4T报表的[!UICONTROL Control vs Targeted]维度的[!UICONTROL Auto-Target]数字不可用。 在添加支持之前，可通过下载[!UICONTROL Lift and Confidence]置信度计算器[来手动计算](https://experienceleague.adobe.com/docs/target/assets/complete_confidence_calculator.xlsx?lang=zh-Hans)。

## 添加量度的体验级别细分

要进一步了解insight如何执行集成ML模型，您可以检查&#x200B;**[!UICONTROL Control vs Targeted]**&#x200B;维度的体验级别细分。 在[!DNL Analysis Workspace]中，将&#x200B;**[!UICONTROL Target Experiences]**&#x200B;维度拖动到报表中，然后分别划分每个控件维度和目标维度。
...
[!DNL Adobe Analytics]活动访问的[!DNL Target]默认计数方法可能包括用户未与[!DNL Target]活动交互的访问。 这是由于在[!DNL Target]访客上下文中保留[!DNL Analytics]活动分配的方式。 因此，访问[!DNL Target]活动的次数有时可能会虚增，从而导致转化率降低。
...
1. 在&#x200B;**[!UICONTROL Components > Create Segment]**&#x200B;工具栏中选择[!DNL Analysis Workspace]选项。
...
![中的[!DNL Analysis Workspace]](assets/Figure5.png)区段

*图5：使用如下图所示的区段过滤A4T中[!UICONTROL Visits]报告[!UICONTROL Auto-Target]的*&#x200B;量度
...
A4T集成允许[!UICONTROL Auto-Target] ML模型使用&#x200B;*用于*&#x200B;生成性能报表[!DNL Adobe Analytics]的相同转化事件数据进行&#x200B;*培训*。 但是，在训练ML模型时，必须使用某些假设来解释此数据，这些假设与[!DNL Adobe Analytics]中报告阶段所做的默认假设不同。
...
### 具有[!DNL Analytics]独特访问转化率&#x200B;*的*&#x200B;量度

**定义具有正量度区段的访问**

在您选择&#x200B;*最大化唯一访问转化率*&#x200B;作为优化标准的情况下，转化率的正确定义是指标值为正的访问次数的部分。 可通过以下方法来实现这一点：创建区段过滤以找出指标为正值的访问次数，然后过滤访问次数量度。

1. 与以前一样，在&#x200B;**[!UICONTROL Components > Create Segment]**&#x200B;工具栏中选择[!DNL Analysis Workspace]选项。
...
1. 在&#x200B;**[!UICONTROL Components > Create Metric]**&#x200B;工具栏中选择[!DNL Analysis Workspace]选项。
...
1. 从&#x200B;**[!UICONTROL Add > Container]**&#x200B;部分的右上角选择&#x200B;**[!UICONTROL Definition]**。
1. 选择两个容器之间的除(÷)运算符。
...
## 摘要： [!DNL Analysis Workspace]报告的最终示例[!UICONTROL Auto-Target]面板
...
在Analysis Workspace的![A4T报表中[!DNL Analysis Workspace]](assets/Figure10.png "最终A4T报表"){width="600" zoomable="yes"}

*图10： [!UICONTROL Auto-Target] [!DNL Adobe Analytics]中的最终A4T [!DNL Workspace]报告，它合并了本教程前面部分中描述的对量度定义的所有调整。*
