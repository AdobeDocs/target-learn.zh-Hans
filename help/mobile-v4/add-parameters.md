...
您可以在[!UICONTROL 设置] > [!UICONTROL 属性]下的[!DNL Target]界面中检索您的at_property值。  将鼠标悬停在属性上，选择代码片段图标并复制`at_property`值：

![复制at_property](assets/at_property_interface.jpg)

将其添加为预回迁请求中每个位置的参数，如下所示：
![添加at_property参数](assets/params_at_property.jpg)
以下是`targetPrefetchContent()`函数的更新代码（请务必更新&#x200B;_[!UICONTROL 此处的at_property值]_&#x200B;占位符文本！）：
...
![将参数添加到实时位置请求](assets/parameters_live_location.jpg)
