...
>[!IMPORTANT]
>
>对于[!UICONTROL 自动定位]活动，您必须检查[!DNL Analytics Workspace]中的报表并手动创建A4T面板。

[!DNL Auto-Target]活动的[!UICONTROL Analytics for Target] (A4T)集成使用[!DNL Adobe Target]集成机器学习(ML)算法，根据每位访客的个人资料、行为和上下文选择最适合该访客的体验，而所有这些操作都使用[!DNL Adobe Analytics]目标量度。

尽管[!DNL Adobe Analytics] [!DNL Analysis Workspace]中提供了丰富的分析功能，但由于实验活动（手动[!UICONTROL A/B测试]和[!UICONTROL 自动分配]）和个性化活动（[!UICONTROL [!UICONTROL 自动定位]]）之间存在差异，因此仍需要对默认&#x200B;**[!UICONTROL Analytics for Target]**&#x200B;面板进行一些修改才能正确解释[!DNL Auto-Target]活动。
...
要为[!UICONTROL 自动定位]报告创建A4T，请从[!DNL Analysis Workspace]中的&#x200B;**[!UICONTROL Analytics for Target]**&#x200B;面板开始（如下所示），或以自由格式表开始。 然后，进行以下选择：

1. **[!UICONTROL 控制体验]**：您可以选择任何体验；但是，稍后将覆盖此选择。 请注意，对于[!UICONTROL 自动定位]活动，控制体验实际上是一种控制策略，它适用于a)在所有体验中随机提供，或b)提供单个体验（此选择在[!DNL Adobe Target]中的活动创建时作出）。 即使您选择了选项(b)，您的[!UICONTROL 自动定位]活动也会指定特定体验作为控制。 您仍然应该按照本教程中概述的方法分析[!UICONTROL 自动定位]活动的A4T。
2. **[!UICONTROL 标准化量度]**：选择[!UICONTROL 访问次数]。
3. **[!UICONTROL 成功量度]**：尽管您可以选择要报告的任何量度，但通常应查看与在[!DNL Target]中创建活动期间选择优化的同一量度有关的报告。

   ![[!UICONTROL Analytics for Target]面板为[!UICONTROL 自动定位]活动设置。](assets/Figure1.png)

   *图1：[!UICONTROL 自动定位]活动的[!UICONTROL Analytics for Target]面板设置。*

>[!TIP]
>
>要为[!UICONTROL 自动定位]活动设置[!UICONTROL Analytics for Target]面板，请选择任意控制体验，选择[!UICONTROL 访问次数]作为标准化量度，并选择在创建[!DNL Target]活动期间为优化选择的相同目标量度。

## 使用[!UICONTROL 控件与目标]维度，用于将[!DNL Target]组合ML模型与您的控件进行比较

默认A4T面板专为经典（手动） [!UICONTROL A/B测试]或[!UICONTROL 自动分配]活动设计，其目标是比较各个体验与控制体验的性能。 但是，在[!UICONTROL 自动定位]活动中，第一顺序比较应在控制&#x200B;*策略*&#x200B;和目标&#x200B;*策略*&#x200B;之间。 换句话说，确定[!UICONTROL 自动定位]组合ML模型相对于控制策略的整体性能的提升。
...
>[!TIP]
>
>在[!DNL Analysis Workspace]中，单击齿轮图标以隐藏[!UICONTROL 转化率]列中的百分比，以帮助将焦点集中在体验转化率上。 转换率随后将格式化为小数，但会相应地将其解释为百分比。

## 为什么“[!UICONTROL 访问]”是[!UICONTROL 自动定位]活动的正确标准化量度

分析[!UICONTROL 自动定位]活动时，始终选择[!UICONTROL 访问次数]作为默认的标准化量度。 [!UICONTROL 自动定位]个性化将为访客选择一个体验，每次访问一次（正式地说，每[!DNL Target]会话一次），这意味着向访客显示的体验在每次访问时都可以更改。 因此，如果您使用[!UICONTROL 独特访客]作为标准化量度，单个用户最终可能会看到多个体验（跨不同访问）这一事实可能会导致转化率混乱。
...
如果您希望报告用户实际与[!UICONTROL 自动定位]活动进行交互的访问（通过进入活动、显示或访问事件，或转化），您可以：

1. 创建一个特定区段，使其包含来自所讨论[!DNL Target]活动的点击，然后
1. 使用此区段筛选[!UICONTROL 访问次数]量度。

**创建区段：**

1. 在[!DNL Analysis Workspace]工具栏中选择&#x200B;**[!UICONTROL 组件>创建区段]**&#x200B;选项。
2. 为您的区段指定一个&#x200B;**[!UICONTROL 标题]**。 在下面显示的示例中，该区段名为[!DNL "Hit with specific Auto-Target activity"]。
3. 将&#x200B;**[!UICONTROL Target活动]**&#x200B;维度拖动到区段&#x200B;**[!UICONTROL 定义]**&#x200B;部分。
...
创建区段后，可使用该区段过滤[!UICONTROL 访问次数]量度，以便[!UICONTROL 访问次数]量度仅包含用户与[!DNL Target]活动交互的访问。

**使用此区段过滤[!UICONTROL 访问次数]：**

1. 从组件工具栏中拖动新创建的区段，并将鼠标悬停在&#x200B;**[!UICONTROL 访问次数]**&#x200B;量度标签的基数上，直到显示蓝色的&#x200B;**[!UICONTROL 过滤依据]**&#x200B;提示符。
2. 发布区段。 过滤器将应用于该量度。
...
A4T集成允许[!UICONTROL 自动定位] ML模型使用[!DNL Adobe Analytics]用于&#x200B;*生成性能报表*&#x200B;的相同转化事件数据进行&#x200B;*培训*。 但是，在训练ML模型时，必须使用某些假设来解释此数据，这些假设与[!DNL Adobe Analytics]中报告阶段所做的默认假设不同。
...
1. 单击&#x200B;**[!UICONTROL 编辑]**。
1. 选择&#x200B;**[!UICONTROL 模型]**：**[!UICONTROL 参与率]**，以及&#x200B;**[!UICONTROL 回顾时间范围]**：**[!UICONTROL 访问]**。
...
1. 与之前一样，在[!DNL Analysis Workspace]工具栏中选择&#x200B;**[!UICONTROL 组件>创建区段]**&#x200B;选项。
2. 为您的区段指定一个&#x200B;**[!UICONTROL 标题]**。
...
4. 在区段定义容器的左上方，选择&#x200B;**[!UICONTROL 包含]** **访问**。
...
通过前面几节中对[!UICONTROL 访问]和目标量度所做的修改，您应该对[!DNL Auto-Target]报表面板的默认A4T进行的最终修改是，创建与正确的目标量度与经过适当过滤的“访问”量度之间的正确比率的转化率。

为此，请使用以下步骤创建一个[!UICONTROL 计算量度]：

1. 在[!DNL Analysis Workspace]工具栏中选择&#x200B;**[!UICONTROL 组件>创建量度]**&#x200B;选项。
1. 为您的量度指定&#x200B;**[!UICONTROL 标题]**。 例如，“活动XXX的访问校正转化率”。
1. 选择&#x200B;**[!UICONTROL 格式]** =百分比和&#x200B;**[!UICONTROL 小数位]** = 2。
1. 将活动的相关目标量度（例如[!UICONTROL 活动转化]）拖动到定义中，然后使用此目标量度上的齿轮图标将归因模型调整为（参与率|访问），如前所述。
1. 从&#x200B;**[!UICONTROL 定义]**&#x200B;部分的右上角选择&#x200B;**[!UICONTROL 添加>容器]**。
1. 选择两个容器之间的除(÷)运算符。
1. 在本教程中，将您之前创建的名为“具有特定[!UICONTROL 自动定位]活动的点击”的区段拖放到该特定[!DNL Auto-Target]活动中。
1. 将&#x200B;**[!UICONTROL 访问次数]**&#x200B;指标拖动到区段容器中。
1. 单击&#x200B;**[!UICONTROL 保存]**。
...
>[!IMPORTANT]
>
>A4T面板中的[!UICONTROL 转化]比率量度未链接到表中的转化事件或标准化量度。 进行本教程中建议的修改时，[!UICONTROL 转化]率不会自动适应这些更改。 因此，如果您修改转化事件归因或标准化量度（或两者），则必须记住的最后一步是修改[!UICONTROL 转化]率，如上所示。

## 摘要： [!UICONTROL 自动定位]报告的最终示例[!DNL Analysis Workspace]面板

将以上所有步骤合并到一个面板中，下图显示了[!UICONTROL 自动定位] A4T活动的推荐报告的完整视图。 此报表与[!DNL Target] ML模型用于优化目标量度的报表相同。 该报告包含本教程中讨论的所有细微差别和建议。 此报表也最接近传统[!DNL Target]报表驱动的[!UICONTROL 自动定位]活动中使用的计数方法。
