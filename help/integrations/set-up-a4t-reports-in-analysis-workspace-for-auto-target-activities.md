...
本文将介绍在Workspace中分析[!DNL Auto-Target]活动的建议修改，这些修改基于以下主要概念：
...
* Adobe Target的ML模型在其培训阶段使用访问 — 回顾范围的归因（在指定的归因模型上也称为“访问回顾窗口”），在划分目标量度时最好使用相同的（非默认）归因模型。

## 在Workspace中为[!DNL Auto-Target]面板创建A4T

要为[!DNL Auto-Target]报表创建A4T，请使用Workspace中的“目标分析”面板（如下所示）进行开始，或以自由格式表开头。 然后进行以下选择：

1. **[!UICONTROL 控制体验]**:您可以选择任何体验；但是，我们稍后将覆盖此选项。请注意，对于[!DNL Auto-Target]活动，控制体验实际上是一种控制策略，即a)在所有体验中随机提供，或b)提供单个体验(此选项是在Adobe Target的活动创建时做出的)。 即使您选择了选择(b) — 您的[!DNL Auto-Target]活动指定了特定体验作为控件 — 您仍应按照本文中概述的方法分析[!DNL Auto-Target]活动的A4T。
1. **[!UICONTROL 标准化量度]**:选择访问。
1. **[!UICONTROL 成功量度]**:尽管您可以选择要报告的任何量度，但通常应将报告视图到在Adobe Target中创建活动时选择用于优化的同一量度。
...
请注意，此替换将使A4T面板上的默认提升和置信度计算无效。 为避免混淆，您可以从默认面板中删除这些量度，同时保留以下报表：

![图2.](assets/Figure2.png)
*png图2:建议的活动基准 [!DNL Auto-Target] 报告。此报表已配置为将目标流量（由集成ML模型提供）与您的控制流量*&#x200B;进行比较

>[!NOTE]
>
>目前，对于自动目标的A4T报表，“控制”和“目标”维度，“提升度”和“置信度”号不可用。 在添加支持之前，可以通过下载[置信度计算器](https://experienceleague.adobe.com/docs/target/assets/complete_confidence_calculator.xlsx?lang=en)手动计算提升度和置信度。
...
分析[!DNL Auto-Target]活动时，始终选择“访问”作为默认标准化量度。 [!DNL Auto-Target] “个性化”为每次访问选择一次访客体验(正式，每次Adobe Target会话一次)，这意味着每次访问时向用户显示的体验都可以更改。因此，如果我们使用唯一访客作为标准化量度，则单个用户最终可能会看到多个体验（跨不同访问），这会导致转化率混淆。
...
如表所示，访客级数字明显不一致。 尽管有两种完全唯一的访客，但这并不是每种体验的单独唯一访客的总和。 虽然访客级转化率未必是错的，但如果比较个人体验，访问级转化率可以说更有意义。 从形式上讲，分析单位（“访问”）与决策粘性单位相同，这意味着衡量指标体验级别划分的总和和比较有意义。
...
1. 输入区段的&#x200B;**[!UICONTROL 标题]**。 在以下示例中，区段名为[!DNL "Hit with specific Auto-Target activity"]。
1. 将&#x200B;**[!UICONTROL 目标活动]**&#x200B;维度拖到段&#x200B;**[!UICONTROL 定义]**&#x200B;部分。
1. 使用&#x200B;**[!UICONTROL equals]**&#x200B;运算符。
1. 搜索您的特定目标活动。
1. 选择齿轮图标，然后选择&#x200B;**[!UICONTROL 归因模型>实例]**，如下图所示。
1. 单击&#x200B;**[!UICONTROL 保存]**。

![图5.](assets/Figure5.png)
*png图5:使用此处显示的区段等区段来筛选报表A4T中的“访问量”量 [!DNL Auto-Target] 度*

创建区段后，我们可以使用它来筛选访问量度，以便它仅包括用户与目标活动互动的访问。
...
A4T集成允许使用与在Adobe Analytics中生成性能报告&#x200B;*相同的转换事件数据对[!DNL Auto-Target]的ML模型进行*&#x200B;培训&#x200B;*。*&#x200B;但是，在培训ML模型时，必须使用某些假设来解释此数据，这与在Adobe Analytics的报告阶段所作的默认假设不同。
...
1. 为此特定的[!DNL Auto-Target]活动拖动您之前创建的区段，该区段在本文中名为“Hit with specific [!DNL Auto-Target]活动”。
...
>[!IMPORTANT]
>
>A4T面板中的转化率量度未链接到表中的转换事件或标准化量度。 当您进行本文建议的修改时，转化率不会适应更改。 因此，如果您修改了一个（或两个）转换事件属性和标准化量度，则您必须记住，最后一步还要修改转化率，如上所示。

## 摘要：[!DNL Auto-Target]报告的最终工作区面板示例

将上述所有步骤组合到单个面板中，下图显示了[!DNL Auto-Target] A4T活动的建议报告的完整视图。 此报告与目标的机器学习模型用于优化目标量度的报告相同，并包含本文讨论的所有细微差别和建议。 此报告也与传统目标报告驱动[!DNL Auto-Target]活动中使用的计数方法最接近。
