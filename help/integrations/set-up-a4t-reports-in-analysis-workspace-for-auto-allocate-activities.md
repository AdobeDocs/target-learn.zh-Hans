...
## 优化条件为“每位访客的量度值最大化[!UICONTROL ”]的Analytics量度
...
| --- | --- | --- |
| 最大化[!DNL Analytics]量度的量度值 | <ul><li>删除[!UICONTROL 置信度]指标。</li><li>移除[!UICONTROL 提升（低）]和[!UICONTROL 提升（高）]。 保持[!UICONTROL 提升(Med)]。</li><li>从[!UICONTROL 转化率]列中取消选中百分比表示以避免混淆。 请参阅下面的[A4T的整体指南](#guidance)。</li><li>将[!UICONTROL 转化]比率量度重命名为“量度/访客”。</li></ul> | <ul><li>删除[!UICONTROL 置信度]指标。</li><li>移除[!UICONTROL 提升（低）]和[!UICONTROL 提升（高）]保留[!UICONTROL 提升（中）]。</li><li>从[!UICONTROL 转化率]列中取消选中百分比表示以避免混淆。 请参阅下面的[A4T的整体指南](#guidance)。</li><li>将[!UICONTROL 转化]比率量度重命名为“量度/访客”。</li><li>确保日期和时间范围与您在[!DNL Target]报表中看到的值一致。 请参阅下面的[A4T的整体指南](#guidance)。</li></ul> |

![最大化收入的量度值](/help/integrations/assets/maximize-metric-value-revenue.png)

## 具有“[!UICONTROL 独特访客转化率]”优化标准的[!DNL Analytics]个量度

**定义**： （具有正值的独特访客数）/（独特访客总数）

示例：假设您的优化量度是[!UICONTROL 收入]。 活动中有五个独特访客，其中三个独特访客进行了购买。 在此示例中，此值= （3位访客的[!UICONTROL 收入]为正）/（总共5位独特访客）= 0.6 = 60%。
...
| --- | --- | --- |
| 最大化[!DNL Analytics]量度的转化 | <ul><li>删除[!UICONTROL 置信度]指标。</li><li>删除所有三个[!UICONTROL 提升]指标。</li><li>从[!UICONTROL 转化率]列中取消选中百分比表示以避免混淆。 请参阅下面的[A4T的整体指南](#guidance)。</li></ul> | <ul><li>删除[!UICONTROL 置信度]指标。</li><li>删除所有三个[!UICONTROL 提升]指标。</li><li>创建一个区段以筛选具有正量度值的访客，这些访客查看了分析的活动。 请参阅下面的[创建区段](#segment)。</li><li>替换自动填充的[!UICONTROL 转化率]指标，使其成为具有正指标值的[!UICONTROL 独特访客]和独特访客之间的分值。 请参阅下面的[更新转化率量度](#update-conversion-metric)。</li><li>从[!UICONTROL 转化率]列中取消选中百分比表示以避免混淆。 请参阅下面的[A4T的整体指南](#guidance)。</li><li>确保日期和时间范围与您在[!DNL Target]报表中看到的值一致。 请参阅下面的[A4T的整体指南](#guidance)。</li></ul> |
...
1. 单击左边栏中&#x200B;**[!UICONTROL 区段]**&#x200B;旁边的&#x200B;**“+”号**。

   左边栏中的区段旁有![加号。](/help/integrations/assets/plus-sign.png)

1. 将区段命名为“具有正量度值的访客”。
1. 在&#x200B;**[!UICONTROL Definition]**&#x200B;下，在&#x200B;**[!UICONTROL Include]**&#x200B;旁边，选择&#x200B;**[!UICONTROL 访客]**。
1. 在&#x200B;**[!UICONTROL 定义]**&#x200B;下，选择活动中的优化量度。
...
1. 将[!UICONTROL 独特访客]量度拖放到与“具有正量度值的访客”相同的列中。

   此配置会创建一个区段，其中包含指标值为正的所有独特访客。 在本例中，选择收入大于零的所有独特访客。

#### 更新[!UICONTROL 转化率]量度 {#update-conversion-metric}

1. 如果尚未这样做，请从面板中删除现有的[!UICONTROL 转化率]列，如下所述。
1. 通过单击左边栏中&#x200B;**[!UICONTROL 量度]**&#x200B;部分旁边的“+”号添加量度。
1. 将量度命名为“转化率”，并将其定义为“（[!UICONTROL 具有正量度值的独特访客]）”除以“独特访客”，如下所示。
...
1. 单击齿轮图标，然后取消选中&#x200B;**[!UICONTROL 百分比]**&#x200B;复选框，因为此值可能会导致混淆。
...
| 所需更改 | 目标触发的报告 | A4T面板报告 |
| --- | --- | --- |
| 使用[!DNL Target]转化量度的[!DNL Analytics]报表 | <ul><li>删除[!UICONTROL 置信度]指标。</li><li>移除[!UICONTROL 提升（低）]和[!UICONTROL 提升（高）]。 保持提升(Med)。</li><li>从[!UICONTROL 转化率]列中取消选中百分比表示以避免混淆。 请参阅下面的[A4T的整体指南](#guidance)。</li></ul> | <ul><li>删除[!UICONTROL 置信度]指标。</li><li>移除[!UICONTROL 提升（低）]和[!UICONTROL 提升（高）]。 保持[!UICONTROL 提升(Med)]。</li><li>从[!UICONTROL 转化率]列中取消选中百分比表示以避免混淆。 请参阅下面的[A4T的整体指南](#guidance)。</li><li>确保日期和时间范围与您在[!DNL Target]报表中看到的值一致。 请参阅下面的[A4T的整体指南](#guidance)。</li></ul> |
...
您可以通过单击[!UICONTROL Target]中报告屏幕上的链接来导航到预建[!UICONTROL Analytics for Target]面板（本指南稍后将此类链接称为“[!DNL Target]触发的报告”）。 或者，您也可以在[!DNL Analytics]中构建A4T面板（此部分稍后将介绍详细信息）。
...
1. 要为[!UICONTROL 自动分配]活动报告创建A4T面板，请从[!DNL Analysis Workspace]中的[!UICONTROL Analytics for Target]面板开始，如下所示。
...
1. 进行以下选择：
   ...
   * **成功量度**：选择您在活动创建期间使用的相同（优化）量度。 如果这是[!DNL Target]定义的转化量度，请选择&#x200B;**[!UICONTROL 活动转化]**。 否则，请选择您使用的[!DNL Adobe Analytics]量度。
